            ...
            try:
                ...
                try:
                    ...
                    if verbose():
                        writerr(
                            colored(
                                getSPACER("ERROR processArchiveUrl 2:  " + str(e)),
                                "red",
                            )
                        )
...
def validateArgProcesses(x):
    ...
    if x < 1 or x > 5:
        raise argparse.ArgumentTypeError(
            "The number of processes must be between 1 and 5. Be kind to Wayback Machine (archive.org) and commoncrawl.org! :)"
        )
    return x
...
def validateArgDate(x):
    ...
    if invalid:
        raise argparse.ArgumentTypeError(
            "A valid date/time needs to be passed in YYYYMMDDhhmmss format (or part of, e.g. YYYYMM) ."
        )
    return x
...
def validateArgMimeTypes(x):
    ...
    if invalid:
        raise argparse.ArgumentTypeError(
            "Pass MIME Types separated by a comma, e.g. text/html,text/xml"
        )
    return x
...
def getAlienVaultUrls():
    ...
    try:
        ...
        url = (
            ALIENVAULT_URL.replace("{TYPE}", indicatorType).replace(
                "{DOMAIN}", quote(argsInputHostname)
            )
            + "&page="
        )
...
def getCommonCrawlUrls():
    ...
    try:
        ...
        if verbose():
            if args.filter_responses_only:
                url = (
                    "{CDX-API-URL}?output=json&fl=timestamp,url,mime,status,digest&url="
                    + subs
                    + quote(argsInput)
                    + path
                )
